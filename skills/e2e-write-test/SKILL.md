...
3.5 **アプリの外を触るなら、計装ではなく OS 層を使う**
   ...
   **押せる 74 件のうち `text` を持つのは 11 件**だった。`resolve` も `label` を返す。
   - **`label` は文字による探索ヒントであって、表示名を保証しない。** 文字が見つかることと、
     **その要素への操作が意図したハンドラに届くことは別**（`ugui_tap` は対象の中心で raycast に当たった
     実体から上へ辿る。届くかは中心に何が居るかで決まる。個々のアプリで通るかは未確認）。
   ...
   - 効くのは**並び順で `path` が変わる動的リスト**（導入先のニュース一覧で実測）。
     **文字が UI に無い**（画像でローカライズ）／**文字がコントロールの兄弟の下**／
     **バッジの数字が label になる**構成では効かない。`dump` で実物を見てから使う
