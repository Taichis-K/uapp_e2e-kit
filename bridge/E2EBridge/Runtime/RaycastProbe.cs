...
namespace E2EBridge
{
    ...
    public static class RaycastProbe
    {
        ...
        // blocker は「遮蔽している実オブジェクト」（blockedBy がパスのときだけ非 null）。
        // 呼び出し側はここからコンポーネント名を列挙して「押して退けるものか・待つべきものか」の
        // 判断材料（blockedByComponents）を返す（導入先要望: シールドを連打してしまった）
        public static (bool hittable, string blockedBy, GameObject blocker) Probe(GameObject target, Vector2 screenPos)
        {
            var eventSystem = EventSystem.current;
            if (eventSystem == null)
                return (false, "NO_EVENTSYSTEM", null);
            ...
            if (!IsRaycastTarget(target))
                return (false, "NOT_RAYCASTABLE", null);
            ...
            return (false, HierarchyDumper.GetPath(top.transform), top);
        }
