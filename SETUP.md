...
- **アプリ外（システムダイアログ等）の操作とOS 層スクショが要るときは
  ...
  疎通・スクショ・タップは実機で実測済み、
  **文字入力（/type）・アラート（/alert）・スワイプ（/swipe）も実機で実測済み**

**設計段階で次の制約を織り込んでおく**:

| 制約 | 内容 |
|---|---|
| **アプリの外は、計装だけでは操作できない** | 計装（E2EBridge）は Unity アプリの中で動くので、**外部ブラウザ・システムダイアログ・他アプリは `dump` にも `tap` にも現れない**。**Android にはこのキットの中に逃げ道がある**（`adb` の uiautomator 経由で `ui_tap` / `ui_type`。外部ブラウザでの認証も自動化できる）。**iOS 側の同梱手段は `-OsAgent`（XCUITest ベースの OS レイヤーエージェント）**で、座標タップ・スクショは実機で実測済み。**文字入力（`/type`）・アラート（`/alert`）・スワイプ（`/swipe`）も実機で実測済み**（検索欄に文字が入る／ダイアログのボタンを列挙して押すと消える／ページ送りが起きる。bundleId 省略で 400・非前面で 409 のガードも確認）。**実機の実測値**: 文字入力は Unity の `InputField` に値が入ることを `get` で確認（**タップしてキーボードが出るまで待つこと** ― 出る前に送ると届かない。導入先は 2 秒）／スワイプは `ScrollRect` の `verticalNormalizedPosition` が 0.0 → 0.0737 と動いた／アラートは実在のダイアログを列挙・押下できた（**`available` にはボタン以外のラベルも混じる** ― iOS 26 の位置情報アラートでは「正確な位置情報: オン」というトグルのラベルが**2 回**出た。**一覧から機械的に選ばず、押したいボタン名を自分で決めて渡す**）。**ただしシステムアラートが出ている間、ブリッジは応答しない**（下の行） |
...
| **ブリッジは loopback にしか bind しない** | `BridgeHost` は `IPAddress.Loopback` で待ち受ける（外部から到達できない設計）。**同じ Wi-Fi に置いても繋がらない**ので、USB が使えない場面の代替にはならない。iOS は `iproxy`、Android は `adb forward` の**トンネル前提**で組む |
| **端末ログは後から回収できない（ライブのみ）** | Unity の `Debug.Log` は iOS の syslog に出ないので、**`devicectl --console` を繋いでいた間しか残らない**。`log collect --device-udid` は root を要求して拒否される（導入先が実機で実測。**こちらでは未追試**）。**証跡が要る区間は、先に繋いでから操作する** |
| **USB ポートが塞がるときの逃げ道** | 有線ヘッドセット等で**ケーブルを抜くと、ブリッジも OS エージェントもコンソールも同時に失われる**（1 ポート機）。導入先が使えた回避は「**端末側でスクリーンショットを撮ってもらい、ケーブルを戻してから取り出す**」― `afcclient -u <UDID> ls DCIM/100APPLE` と `... get <名前> <保存先>`（libimobiledevice。**キットには同梱しない**）。`devicectl device copy from` の `--domain-type` は`temporary / appDataContainer / appGroupDataContainer / systemCrashLogs` だけで**写真領域に届かない**（導入先が実機で実測。**こちらでは未追試**） |
| **システムアラートが出ている間、ブリッジは応答しない** | **権限ダイアログ等が出ている間、iOS はアプリを非アクティブにする**ので Unity のメインスレッドが進まず、**ブリッジの ping がタイムアウトする**。**アプリは壊れていない** ― OS 層で閉じれば即座に復帰する。ドライバのタイムアウトは「アプリがフリーズ/ANR」も候補に挙げるが、**この経路ではそれが誤り**。アラートが出うる導線（位置情報・通知・ATT・ローカルネットワーク）では、**異常と読む前にシステムアラートを疑う**。閉じるときは **`os_agent.handle_alert("許可しない")` のようにボタン名を指定する** ― **省略すると先頭が押される**ので、確認のつもりの引数なし呼び出しで意図せず許可してしまう。2026-09-10 に導入先が実機で実測（アラート表示中は ping がタイムアウトし、閉じた直後に復帰） |
...
| **OS エージェントは長丁場では落ちる前提で組む** | XCUITest ランナーが**約 85 分で終了した**実測がある（テスト自体は failure 扱い）。**落ちてもアプリとブリッジは無事**で、ブリッジ側の操作だけで画面を戻せた。長い検証では**途中で OS 層が使えなくなる前提**にし、必要な時に `-OsAgent` を張り直す |
...
2. エミュレーター起動 → `uapp_e2e\scripts\run-e2e.ps1`（テスト未作成なら ping 疎通のみ。
   ...
   **`run-e2e.ps1` の adb 呼び出しには期限がある**（応答しない adb server に掴まったまま
   無言で止まらないため）。**遅いだけの環境で確定失敗にしないよう、延ばす口を用意してある**:

   | 用途 | 既定 | 変える環境変数 |
   |---|---|---|
   | デバイスの接続待ち（`wait-for-device`） | 120 秒 | `UAPP_E2E_ADB_WAIT_DEVICE_SECONDS` |
   | インストール | 600 秒 | `UAPP_E2E_ADB_INSTALL_SECONDS` |
   | アプリ起動（`am start -W`） | 180 秒 | `UAPP_E2E_ADB_START_SECONDS` |

   **`0` は「制限なし」ではない**（即座に打ち切る）ので、数字でない値や `0` を入れると
   警告を出して既定値で続ける。期限を超えたときのメッセージは**観測したことだけ**を書く
   （adb server とデバイスのどちらが原因かは、その時点では区別できない）。
