...
- **ラベルは `label` で見る。`text` ではない。** `text` は**その GameObject 自身の
  ...
  **導入先の実機では、押せる 74 件のうち `text` を持つのは 11 件だった**（2026-09-11）。
  `label` は**自身 → 子孫の順で最初に見つかった文字**を返すので、**呼び手はこれ 1 つだけ見ればよい**
  - **`label` は文字による探索ヒントであって、表示名を保証しない。** 文字が見つかることと、
    **その要素への操作が意図したハンドラに届くことは別**。`ugui_tap` は対象の中心座標で raycast に
    **当たった実体**（対象自身とは限らない ― 中心を覆う子の Image かもしれない）から
    `ExecuteEvents.ExecuteHierarchy` で**上へ**辿る。届く／届かないは**中心に何が居るか**で決まり、
    「label が付いた要素の path を叩けばその要素のハンドラに届く」とは言えない
    （配送機構は `UguiAdapter.cs` で確認。個々のアプリで通るかは未確認）。
    `label` で候補を絞ったら、`resolve` の `hittable` と `interactable` で操作対象として確かめる
  - **子孫から文字を借りるのは「既知のコントロール」だけ** ― 自身に `Selectable`（uGUI）／
    `UIButton`（NGUI）があるもの。判定は**コンポーネントの存在**で、`interactable` の値は見ない。
    子孫探索も**別の既知のコントロールの枝には降りない**（無効化した子ボタンも境界）。
    `Panel > Card(Image) > Text` のように誰もコントロールでない構成では、Text が自身の文字を持つだけで
    Panel も Card も名乗らない／**GameObject が非アクティブな枝**は見ない
    （コンポーネントの `enabled=false` や `alpha=0` で隠した文字は返る。`dump` と同じ扱い）／
    深さは 4 階層まで／同じ深さなら**深さ優先で最初に見つかった**もの。
    **`Selectable` を持たない独自ハンドラ**（`IPointerClickHandler` を直接実装した `Image` 等）は
  ...
  - **`label` は押せる側にだけ付く。** コントロールが子孫の文字を借りたとき、**その子孫の item には
    ...
    items から子を落とすことはしない（`dump` の hittable 集合との一致が契約）
  - **それでも `label` は一意ではない。** 同じ文字のコントロールが 2 つあれば 2 件並ぶ。
    **パス長・深さ・center で機械的に 1 つへ決めない**。候補が複数なら `interactable` の有無と
    `path` の意味で選び、**同名候補が残るときは自動で確定せず失敗させる**
  - **効く構成と効かない構成**: 効くのは**動的リストのように並び順で `path` が変わる要素**
    （導入先のニュース一覧 16 件で実測。これまで安定した指定手段が無かった場所）。
    効かないのは、**文字が UI に無い**（ボタン面を画像でローカライズしている）／
    **文字がコントロールの兄弟の下にある**（`Button` と `Text` が同じ親の兄弟で、親がコントロールでない）／
    **バッジの数字を拾う**（ボタンの配下にバッジがあると "1" "2" のような数字が label になる。
    導入先のフッターで実測）
...
- **重い画面ほど効き、軽い画面では往復ぶん不利**。導入先の実機実測では、重い画面で
  ...
  **この数字は `label` を足す前の測定**（0.1.19 で子孫の走査が加わった）。
  エディタ実測では**走査したノードあたり約 12µs** で、テキストを持たない子孫が多い
  入れ物ほど効く。**デバイスでの再測はしていない**（サンプルは items が 3〜8 件で、
  往復のばらつき数十 ms に沈んで差が出ない）
...
### texts
**指定した型**の表示テキストを、パスと本文だけで返す。
```json
→ {"cmd": "texts", "args": {"types": ["UnityEngine.UI.Text", "UILabel"], "scope": "scene"}}
← {"scene": "SampleScene",
   "scope": "scene",
   "items": [{"path": "Canvas/Title", "text": "設定"},
             {"path": "Persistent/Toast", "text": "保存しました", "dontDestroyOnLoad": true}],
   "resolvedTypes": [{"type": "UnityEngine.UI.Text", "count": 2},
                     {"type": "UILabel", "count": 0}],
   "unknownTypes": []}
```
- **`hittables` は押せる要素のテキストしか返さない**ので、見出し・残数表示・セリフのような
  「押せないが読みたい」ものはこちらで取る（導入先はこれが無くて自前の計装を残していた）

**型も範囲も呼び手が決める。ブリッジは推測しない。**

- **`types` は必須**。既定の型リストは持たない ―
  **使う側は自分の UI がどのクラスで表示しているかを知っている**のに、こちらが推測した既定を使うと
  「TMP を使っているのに `Text` だけ集まって、テキストが無いと読む」が起きる。
  未指定は候補つきのエラーで止まる。使う型は `dump` の `components` に出ている名前がそのまま使える
  （完全修飾名でも短い名前でも引ける）
- **`scope` は任意**（既定 `"scene"`）。探索の起点だけを決める:

| `scope` | 起点 |
|---|---|
| `"scene"`（既定）/ `"all"` | 読み込み済みの全体。**取りこぼさない** |
| `"canvas"` | root Canvas の配下だけ |
| `"ngui"` | NGUI の UIRoot の配下だけ |
| `"<パス>"` | そのオブジェクトの配下だけ（`dump` の `path` と同じ表記。無ければ `NOT_FOUND` で止まる） |

**既定が `scene` なのは、絞るのは速さのためであって正しさのためではない**から。
速さが要る呼び手（何度も叩く walker など）が、**自分の構成を知ったうえで**絞る。
**型から置き場所を決めつけない** ― 3D メッシュ系が Canvas の下にいることもあれば、
uGUI を独自のルートでまとめていることもある。だから**型ごとに起点を変える表は持たない**
（一度そう作って捨てた。決めつけた側が必ず取りこぼす）。
**絞った結果が空でも、黙って全体へは広げない** ― 広げると
「絞ったつもりが効いていない」ことに気づけない。

- **`DontDestroyOnLoad` 配下も対象**。常駐しているものには `dontDestroyOnLoad: true` が付く
  （`dump` のノードと同じ印）。**実プロジェクトの常駐 UI はここに居る**ので、印が無いと
  「シーン遷移したのに同じ文字列が出ている」を読み違える
- **解決できなかった型は `unknownTypes` に返る**。黙って空を返すと、型名を打ち間違えただけなのに
  「テキストが無い」と読めてしまう＝**偽の緑**。
  **`Component` でない型も「解決できなかった」に入る**（`texts` が集められるのは Component だけ。
  例: `System.String` / `Vector3`）
- **短い名前で `Component` の型が複数当たったら `AMBIGUOUS` で止まる**（候補を完全修飾名で返す）。
  **列挙順に依存して黙って 1 つ選ぶのが一番まずい** ― 呼び手は「なぜか集まらない」ことにしか
  気づけない。以前は名前が一致した型を無条件に返しており、
  **`Text` のように非 `Component` の同名が別アセンブリに居ると `INTERNAL` で落ちていた**
  （`System.Net.Mime.MediaTypeNames+Text` / `System.Xml.Xsl.Xslt.Text` /
  `UnityEngine.UI.Text` の 3 件が同名。`UILabel` や `TextMesh` は通るので
  **通ったり落ちたりする**という悪い形だった）。
  **実在の衝突例**: `DropdownItem` は `TMPro.TMP_Dropdown+DropdownItem` と
  `UnityEngine.UI.Dropdown+DropdownItem` の入れ子型で衝突する
  ＝ **TMP と uGUI を両方入れている実プロジェクトなら踏みうる**
  （入れ子型は `+` 表記の完全修飾名でそのまま指定できる）
- **`unknownTypes` は続行し、`AMBIGUOUS` は呼び出し全体を止める** ―
  **「無いと確定したもの」と「間違ったものを返すかもしれないもの」を分けている**。
  複数の型をまとめて投げたとき、1 つが曖昧なら**他の型のぶんも返らない**ので注意
  （曖昧なまま「たぶんこれ」を返すより、指定を直してもらうほうが安全という判断）
- **空文字も返す**（空になっていること自体が見たい情報）。非アクティブは返さない。
  1 つの GameObject が複数の対象型を持っていても 1 件
- **`dump` との集合一致は契約ではない**。`dump` の `text` は名前の部分一致で拾う**別の判定**で、
  階層を全部見せる口なので型を絞れないため ― **用途が違うので判定も違ってよい**
...
- **`input_reset` はタッチのポインタも解放する**（`releasedPointers`）。
  以前はキー・マウス・パッドしか見ておらず、**押されたまま残ったポインタが解放されなかった** ―
  異常終了（常駐スクリプトを kill する等）で押下が残ると以後のタップが全部
  `POINTER_ALREADY_DOWN` で落ちるのに、`input_reset` は `released: 0` を返すだけだった。
  **症状は「特定のボタンが効かない」に見える**ので、導入先はアプリ側の不具合を探しに行きかけた。
  件数は**由来ごとに分けて返す**（合算すると何が残っていたのかが消える）
