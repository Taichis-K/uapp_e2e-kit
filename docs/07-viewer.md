...
**設定（`e2e-config.json`）は「記録した時に使ったもの」を最優先する**。`run-e2e.ps1` は記録時に
`UAPP_E2E_CONFIG_PATH` で設定の実パスを渡し、それが `journey.json` の `configPath` に残る。
`serve` はまずその記録を使うので、**注入モードのようにキットのハブと対象プロジェクトが
別の場所にある構成でも、対象側の `uiType` と `editorBridgePort` が使われる**（issue #41。
以前はハブ側の設定を拾い、`ngui_tap` を使うべき画面で `tap` を送るなどの食い違いが起きた）。
記録が無い（旧い journey）・実在しないパス・壊れた `journey.json` のときは、
**従来どおり journey ディレクトリを起点に上位探索する**。
