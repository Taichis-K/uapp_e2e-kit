...
| `ios` | **ビルドのための恒久付与は不要**（`build-ios.ps1` の BuildEntry がビルド時に一時付与・復元する）。**例外はエディタ直結**: iOS プラットフォームのまま Play で使うなら iOS ターゲットへの付与が要る（エディタはアクティブターゲットの define でコンパイルするため。Windows でも可）。付与されている場合は注意が表示される — **BuildEntry を通さない本番 iOS ビルドには計装が混入する**ので、本番前に外す運用を明確に（**運用に頼らず機械で止めたいなら下の「本番混入ガード」**） |
...
define を**恒久付与**する構成では、**本番ビルドにも計装が入りうる**。
運用（本番前に外す）だけが歯止めなので、**機械で止めたいとき**は
`e2e-config.json` に**開発版ビルドの目印にしている define** を書く:

```json
"productionGuard": { "devDefine": "MYPROJ_DEV" }
```

これを書くと、`UAPP_E2E_BRIDGE` が付いているのに `devDefine` が無いビルドが
**プリプロセス段階で失敗する**（`IPreprocessBuildWithReport`）。

- **既定では何もしない。** 空のままなら検査は動かない ―
  **キットが導入先のビルドへ勝手に割り込まないため**
- **止めるのは「計装は入るのに開発版の目印が無い」ときだけ。**
  計装が入らないビルドは素通しする（ここで欲を出すと、誤検知でリリースを止める側になる）
- **外し方**: 環境変数 `UAPP_E2E_SKIP_PRODUCTION_GUARD=1`。
  失敗メッセージにも書いてある（**外し方が分からない検査は、本番リリースを止める**）
- 設定を読めなくても**ビルドは止めない**（警告だけ）。保険が事故の原因にならないように
